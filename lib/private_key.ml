...
module Asn = struct
  ...
  let ec_priv_to_cs ?curve ?pub key = ec_priv_to_cs (1, key, curve, pub)
  ...
  let unparse_private p =
    let alg, cs =
      match p with
      | `RSA pk -> Algorithm.RSA, rsa_priv_to_cs pk
      | `ED25519 pk ->
        Algorithm.ED25519, ed25519_to_cs (Hacl_ed25519.encode_priv pk)
      | `P224 pk ->
        Algorithm.EC_pub `SECP224R1,
        ec_priv_to_cs (Mirage_crypto_ec.P224.Dsa.priv_to_cstruct pk)
      | `P256 pk ->
        Algorithm.EC_pub `SECP256R1,
        ec_priv_to_cs (Mirage_crypto_ec.P256.Dsa.priv_to_cstruct pk)
      | `P384 pk ->
        Algorithm.EC_pub `SECP384R1,
        ec_priv_to_cs (Mirage_crypto_ec.P384.Dsa.priv_to_cstruct pk)
      | `P521 pk ->
        Algorithm.EC_pub `SECP521R1,
        ec_priv_to_cs (Mirage_crypto_ec.P521.Dsa.priv_to_cstruct pk)
    in
    (0, alg, cs)
